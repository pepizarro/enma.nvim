...
local colors = require("enma.colors")
...
function M.setup()
  ...
  theme.highlights = {
    ...
    Cursor = { fg = c.base04, bg = c.purple100 }, -- character under the cursor
    ...
    CursorColumn = { bg = c.sidebar_bg }, -- Screen-column at the cursor, when 'cursorcolumn' is set.
    CursorLine = { bg = c.sidebar_bg, sp = c.base1 }, -- Screen-line at the cursor, when 'cursorline' is set.  Low-priority if foreground (ctermfg OR guifg) is not set.
    ...
    VertSplit = { fg = c.sidebar_bg }, -- the column separating vertically split windows
    ...
    SignColumn = { fg = c.base0, bg = c.sidebar_bg }, -- column where |signs| are displayed
    SignColumnSB = { fg = c.base0, bg = c.sidebar_bg }, -- column where |signs| are displayed
    Substitute = { fg = c.base04, bg = c.red500 }, -- |:substitute| replacement text highlighting
    LineNr = { fg = c.purple300, bg = c.sidebar_bg }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    CursorLineNr = { fg = c.purple, bg = c.purple500, bold = true }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    MatchParen = { fg = c.green100, bg = c.green900, bold = true }, -- The character under the cursor or just before it, if it is a paired bracket, and its match. |pi_paren.txt|
    ...
    Normal = { fg = c.white100, bg = c.bg }, -- normal text
    ...
    Pmenu = { fg = c.purple, bg = c.sidebar_bg }, -- Popup menu: normal item.
    PmenuSel = { fg = c.base2, bg = c.green700 }, -- Popup menu: selected item.
    PmenuSbar = { fg = c.green700, reverse = true }, -- Popup menu: scrollbar.
    PmenuThumb = { fg = c.green500, reverse = true }, -- Popup menu: Thumb of the scrollbar.
    ...
    Search = { fg = c.purple300, reverse = true }, -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand out.
    IncSearch = { fg = c.green300, standout = true }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    ...
    SpellBad = { sp = c.red300, undercurl = true }, -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
    ...
    StatusLine = { fg = c.green100, bg = c.green900 }, -- status line of current window
    StatusLineNC = { fg = c.green100, bg = c.green900 }, -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
    ...
    Visual = { fg = c.black, bg = c.base01 }, -- Visual mode selection
    ...
    Constant = { fg = c.green100 }, -- (preferred) any constant
    String = { fg = c.green90 }, --   a string constant: "this is a string"
    Character = { link = "Constant" }, --  a character constant: 'c', '\n'
    -- Number        = { }, --   a number constant: 234, 0xff
    -- Boolean       = { }, --  a boolean constant: TRUE, false
    -- Float         = { }, --    a floating point constant: 2.3e10

    Identifier = { fg = c.white400, style = options.styles.variables }, -- (preferred) any variable name
    Function = { fg = c.violet400, style = options.styles.functions }, -- function name (also: methods for classes)

    Statement = { fg = c.purple100 }, -- (preferred) any statement
    -- Conditional   = { }, --  if, then, else, endif, switch, etc.
    -- Repeat        = { }, --   for, do, while, etc.
    -- Label         = { }, --    case, default, etc.
    Operator = { fg = c.purple }, -- "sizeof", "+", "*", etc.
    Keyword = { fg = c.purple100, style = options.styles.keywords }, --  any other keyword
    -- Exception     = { }, --  try, catch, throw

    PreProc = { fg = c.purple200 }, -- (preferred) generic Preprocessor
    -- Include       = { }, --  preprocessor #include
    -- Define        = { }, --   preprocessor #define
    -- Macro         = { }, --    same as Define
    -- PreCondit     = { }, --  preprocessor #if, #else, #endif, etc.

    Type = { fg = c.green50 }, -- (preferred) int, long, char, etc.
    -- StorageClass  = { }, -- static, register, volatile, etc.
    -- Structure     = { }, --  struct, union, enum, etc.
    -- Typedef       = { }, --  A typedef

    Special = { fg = c.purple200 }, -- (preferred) any special symbol
    ...
    Todo = { fg = c.green200, bg = c.green700, bold = true }, -- (preferred) anything that needs extra attention; mostly the keywords TODO FIXME and XXX
    ...
    DiagnosticWarn = { fg = c.warning, bg = c.sidebar_bg }, -- Used as the base highlight group. Other Diagnostic highlights link to this by default
    DiagnosticInfo = { fg = c.info, bg = c.sidebar_bg }, -- Used as the base highlight group. Other Diagnostic highlights link to this by default
    DiagnosticHint = { fg = c.hint, bg = c.sidebar_bg }, -- Used as the base highlight group. Other Diagnostic highlights link to this by default
    DiagnosticUnnecessary = { fg = c.base00, bg = c.sidebar_bg }, -- Used as the base highlight group. Other Diagnostic highlights link to this by default
    ...
    ["@comment.todo"] = { fg = c.green50, bg = c.green700 },
    ...
    ["@operator"] = { fg = c.white400 }, -- For any operator: `+`, but also `->` and `*` in C.

    --- Punctuation
    ["@punctuation.delimiter"] = { fg = c.base1 }, -- For delimiters ie: `.`
    ["@punctuation.bracket"] = { fg = c.violet300 }, -- For brackets and parens.
    ["@punctuation.special"] = { fg = c.cyan100 }, -- For special punctutation that does not fall in the catagories before.
    ...
    --- Functions
    ["@constructor"] = { fg = c.base2 }, -- For constructor calls and definitions: `= { }` in Lua, and Java constructors.
    ["@variable.parameter"] = { fg = c.red200 }, -- For parameters of a function.
    ["@variable.parameter.builtin"] = { fg = c.violet300 }, -- For builtin parameters of a function, e.g. "..." or Smali's p[1-99]

    --- Keywords
    ["@keyword"] = { fg = c.purple200, style = options.styles.keywords }, -- For keywords that don't fall in previous categories.
    ["@keyword.function"] = { fg = c.purple100, style = options.styles.functions }, -- For keywords used to define a fuction.
    ...
    -- console.log, vim, etc
    ["@variable.builtin"] = { fg = c.purple100 }, -- Variable names that are defined by the languages, like `this` or `self`.
    ...
    ["@tag.delimiter.tsx"] = { fg = c.green500 },
    ["@tag.tsx"] = { fg = c.green500 },
    ["@keyword.return.tsx"] = { fg = c.yellow500, style = "italic" },
    ["@keyword.tsx"] = { fg = c.yellow500, style = "italic" },

    -- typescript
    ["@variable.typescript"] = { fg = c.base2 },
    ...
    -- javascriptreact
    ["@keyword.javascript"] = { fg = c.purple100 }, -- export,
    ["@keyword.return.javascript"] = { fg = c.purple200, style = "italic" },
    ["@tag.delimiter.javascript"] = { fg = c.green500 }, -- <> tag in js
    ["@tag.builtin.javascript"] = { fg = c.orange90 }, -- html tags
    ["@tag.attribute.javascript"] = { fg = c.orange90 }, -- class, id, etc
    ["@variable.javascript"] = { fg = c.purple },
    ["@tag.javascript"] = { link = "Type" },
    ["@number.javascript"] = { fg = c.green200 },

    -- ["@function.call.javascript"] = { link = "@function" },
    ["@lsp.typemod.function.declaration.javascript"] = { fg = c.yellow500, style = "bold" },
    ["@lsp.type.parameter.javascript"] = { fg = c.orange200 },

    -- Golang
    ["@function.go"] = { fg = c.blue, style = "bold" },
    -- ["@type.definition.go"] = { fg = c.blue, style = "bold" },
    ...
    -- GitSigns
    GitSignsAdd = { fg = c.green500, bg = c.sidebar_bg }, -- diff mode: Added line |diff.txt|
    GitSignsChange = { fg = c.yellow500, bg = c.sidebar_bg }, -- diff mode: Changed line |diff.txt|
    GitSignsDelete = { fg = c.red500, bg = c.sidebar_bg }, -- diff mode: Deleted line |diff.txt|

    -- Telescope
    TelescopeBorder = { fg = c.green900, bg = c.bg_float },
    ...
    MiniStatuslineDevinfo = { fg = c.green700, bg = c.bg_highlight },
    MiniStatuslineFileinfo = { fg = c.green500, bg = c.bg_highlight },
    MiniStatuslineFilename = { fg = c.green, style = "bold", bg = c.bg_highlight },
    MiniStatuslineInactive = { fg = c.blue, bg = c.bg_statusline },
    MiniStatuslineModeCommand = { fg = c.black, bg = c.yellow, bold = true },
    MiniStatuslineModeInsert = { fg = c.black, bg = c.purple100, bold = true },
    MiniStatuslineModeNormal = { fg = c.black, bg = c.green, bold = true },
